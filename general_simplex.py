...
class GeneralSimplexSolver:
    def __init__(self, equals_A: list, lesses_A: list, greaters_A: list,
                 equals_b: list, lesses_b: list, greaters_b: list,
        ...
        self.__len_greaters = len(greaters_b)
        if len(greaters_b) > 0: self.__has_phase_one = True
        self.__m = len(lesses_b) + len(greaters_b) * 2
        self.__n = len(c) + len(unrestricts)
        self.__a = []
        self.combine_As(equals_A, lesses_A, greaters_A)
        self.add_complimentaries_for_unrestricts(c, unrestricts)
        self.__a = FractionMatrix.from_matrix(self.__a)
        self.add_slacks_and_artificials(len(equals_b), len(lesses_b), len(greaters_b))
        self.merge_bs(equals_b, lesses_b, greaters_b)
        self.__a.rbind(
            FractionMatrix.from_rows(c).merge(FractionMatrix.zeroes(1, self.__m + 1))
        )
        if not maximize: self.__a.row_multiplication(-1, -1)
        if self.__has_phase_one: self.construct_phase_one(len(greaters_b))
    ...
    def add_slacks_and_artificials(self, len_equals: int, len_lesses: int, len_greaters: int):
        slacks = FractionMatrix.identity(len_lesses + len_greaters)
        for i in range(len_greaters):
            k = len_lesses + i
            slacks[k][k] = slacks[k][k] * -1
        artificials = FractionMatrix.identity(len_greaters)
        combined = slacks.merge(FractionMatrix.zeroes(len_lesses, len_greaters).rbind(artificials))
        self.__a.merge(FractionMatrix.zeroes(len_equals, self.__m).rbind(combined))
    ...
    def construct_phase_one(self, len_greaters: int):
        artificials_sum = [0] * self.__a.n
        for i in range(len_greaters):
            artificials_sum[self.__n + self.__m - len_greaters + i] = -1
        self.__a.rbind(FractionMatrix.from_rows(artificials_sum))
        for i in range(len_greaters):
            self.__a.row_addition(-1, -len_greaters + i - 2, 1)
    ...
    def solve(self):
        if self.__has_phase_one:
            ...
            for i in range(self.__len_greaters): self.__a.remove_col(-2)
            self.__m -= self.__len_greaters
            self.__has_phase_one = False
        self.__solve()
